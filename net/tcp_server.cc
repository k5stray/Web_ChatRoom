...
// ====================== EventLoop Start ====================== //
EventLoop::EventLoop() : epfd_(epoll_create1(0)), looping_(false)
{
...
bool EventLoop::mod_channel(Channel * ch)
{
    epoll_event e{};
    e.events = ch->events_;
    e.data.ptr = ch;
...
bool EventLoop::del_channel(Channel * ch)
{
...
void Acceptor::check_inactive_connect(uint64_t seconds)
{
    std::lock_guard<std::mutex> lock(conn_mtx_);
    for(auto it = conn_list_.begin(); it != conn_list_.end(); ) {
        if((*it)->check_inactive_timeout(seconds)) {
            (*it)->stop_conn();
            it = conn_list_.erase(it);
        } else{
            ++it;
        }
    }
...
// ====================== TcpConnection Start ====================== //
TcpConnection::TcpConnection(EventLoop* loop, int fd) : fd_(fd), closed_(false), loop_(loop)
{}
...
void TcpConnection::enable_write()
{
    ...
    if (!loop_->mod_channel(ch_)) {
        LOG_ERROR_("Fail to enable write, close fd:%d\n", ch_->fd_);
        close_conn();
    }
}
...
void TcpConnection::disable_write()
{
    ch_->events_ = EPOLLIN | EPOLLRDHUP;
    if (!loop_->mod_channel(ch_)) {
        LOG_ERROR_("Fail to disable write, close fd:%d\n", ch_->fd_);
        close_conn();
    }
}
...
bool TcpConnection::check_inactive_timeout(uint64_t seconds)
{
    uint64_t sec = (uint64_t)std::chrono::duration<double>
                    (Clock::now() - last_recv_time_).count();
    if (sec > seconds) {
        LOG_WARN_("Close invactive connect, fd:%d\n", fd_);
        return true;
    }
    return false;
}
...
// ====================== TcpServer Start ====================== //
TcpServer::TcpServer(int ioThreads) : io_pool_(ioThreads) {}
...
void TcpServer::check_inactive_connect(uint64_t seconds)
{
    for (auto &elem : acceptors_) {
        elem->check_inactive_connect(seconds);
    }
}
